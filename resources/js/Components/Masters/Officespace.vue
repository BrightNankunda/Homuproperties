<template>
  <div>
    ...
    <div class="row hostels my-5" v-else>
      <div v-for="image in filteredOfficespace" :key="image.id" class="col-md-6 col-lg-3">
...
export default {
  ...
  computed: {
    filteredOfficespace() {
      if (this.filter == "kampala") {
        ...
        return this.$store.getters.officespace.filter((officespace) => officespace.location === "Kira");
      } else if (this.filter == "ntinda") {
        return this.$store.getters.officespace.filter((officespace) => officespace.location === "Ntinda");
      } else if (this.filter == "entebbe") {
        return this.$store.getters.officespace.filter((officespace) => officespace.location === "entebbe");
      } else if (this.filter == "makindye") {
        return this.$store.getters.officespace.filter((officespace) => officespace.location === "makindye");
      } else if (this.filter == "mukono") {
        return this.$store.getters.officespace.filter((officespace) => officespace.location === "Mukono");
      } else if (this.filter == "kawempe") {
        return this.$store.getters.officespace.filter((officespace) => officespace.location === "Kawempe");
      } else {
  ...
  mounted() {
    this.$store.dispatch("officespace").then(response =>{
      this.loading = false
    })
  },
};
