<template>
  <div>
    ...
    <div class="row hostels my-5" v-else>
      <div v-for="image in filteredRentals" :key="image.id" class="col-md-6 col-lg-3">
...
export default {
  ...
  computed: {
    filteredRentals() {
      if (this.filter == "kampala") {
        ...
        return this.$store.getters.rentals.filter((rental) => rental.location === "Kira");
      } else if (this.filter == "ntinda") {
        return this.$store.getters.rentals.filter((rental) => rental.location === "Ntinda");
      } else if (this.filter == "entebbe") {
        return this.$store.getters.rentals.filter((rental) => rental.location === "entebbe");
      } else if (this.filter == "makindye") {
        return this.$store.getters.rentals.filter((rental) => rental.location === "makindye");
      } else if (this.filter == "mukono") {
        return this.$store.getters.rentals.filter((rental) => rental.location === "Mukono");
      } else if (this.filter == "kawempe") {
        return this.$store.getters.rentals.filter((rental) => rental.location === "Kawempe");
      } else {
  ...
  mounted() {
    this.$store.dispatch("rentals").then(response=> {
      this.loading=false
    })
  },
};
